...
- 基于浏览器的交互终端，支持终端尺寸同步。
- 本地保存和管理连接配置。
- SFTP 批量上传、下载和远程文件浏览；下载可通过新版 Chrome 或 Edge 的原生目录选择器直接保存至指定本地目录。
- 已完成上传和下载的传输历史，支持搜索、查看位置与服务器信息，以及删除单条或清空记录。
...
连接配置、用户偏好和自定义背景图片均保存在 Docker 命名卷 `webssh-data`，更新、重建容器或执行 `docker compose down` 后仍会保留。传输历史会保存在浏览器本地和服务端，当前 Compose 配置未将其单独挂载到命名卷。
