...
- ⬇️ 下载列表支持勾选多个文件；一次选择本地目标文件夹后统一保存
- ⬆️ 上传弹窗支持一次选择多个本地文件，并将其批量上传到当前远程目录
- 🎨 统一上传、下载弹窗的目录栏、底部操作栏和浅色模式配色

### 🛡️ 稳定性与修复

- 🔄 目录读取加入请求与会话隔离，避免切换目录、会话或关闭弹窗后出现过期列表覆盖
- ✅ 强化上传数据完整性校验，阻止超额或不完整数据被错误标记为上传成功
- 📄 下载前校验远程目标为普通文件，避免目录及特殊文件导致传输异常
- ⌨️ 补齐 Escape 对上传、下载和冲突弹窗的关闭处理
- 🧹 移除已废弃的上传目录验证协议及前端状态逻辑
