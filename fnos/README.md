...
- 下载时可选择本地目标目录，文件分片会直接写入该目录，不再经过浏览器默认下载目录（需使用新版 Chrome 或 Edge）；
- 优化传输历史的展示与检索，补充文件位置、服务器和传输信息，并支持删除单条或清空记录；
- 修复下载完成状态大小、取消下载和关闭会话时的本地写入流清理。
...
- 传输历史：已完成传输的记录会保存在浏览器本地和服务端，最多保留 200 条；可在应用中搜索、删除或清空；
- 卸载时可选择保留或删除应用数据。
